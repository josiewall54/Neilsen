...
%% Argument handling
    ...
    if size(binnedData.timeframe,1) == 1
        binsize = double(binnedData.timeframe(1));
    else
...
%             H=[H,filMIMO4(Inputs,Outputs,numlags,numsides,1)];
            H=[H,filMIMO4vMCT(Inputs,Outputs,numlags,numsides,1)];  % line modified by MCT 8/13/17
%             [PredictedData,spikeDataNew,ActualDataNew]=predMIMO4(Inputs,H,numsides,fs,Outputs);
           okind = find(~isnan(Inputs(:,1)));
           [PredictedData,spikeDataNew,ActualDataNew]=predMIMO4MCT(Inputs,H,numsides,fs,Outputs);
        end
...
%         okind = find(~isnan(PredictedData(:,1)));
%         PredictedData = PredictedData(okind,:);
%         ActualDataNew = ActualDataNew(okind,:);
...
%                 PredictedData(~IncludedDataPoints,z)= patch(z);
                ...
                okind = find(~isnan(PredictedData(:,z)));
                [P(:,z)] = WienerNonlinearity(PredictedData(okind,z), ActualDataNew(okind,z), options.PolynomialOrder);

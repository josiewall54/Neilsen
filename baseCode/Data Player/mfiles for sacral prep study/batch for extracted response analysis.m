...
scale = 1;   % this is the display gain
filen = 1;
...
for ii = 1:size(allanswers,1) 
    for jj = 1:size(allanswers,2)                
        if ~strcmp(allanswers{ii,jj},'Y')  % anything other then Y means blank out the trial
            intresp(:,ii,jj) = NaN;
            ntotal(:,ii,jj) = NaN;
        end
    end
...
for mm = 1:size(results{1},1)
    ...
    xlswrite(spreadsheetname,squeeze(intresp(mm,:,:)),sheetname);
%     sheetname = ['# events above thresh' num2str(mm)];
%     xlswrite(spreadsheetname,squeeze(nabove(mm,:,:)),sheetname);
end


%% reviews the 'N' trials identified in allanswers, and let's the user
% change them
% need to load the results, calculate the intresp as above, and load
% allanswers
%
% creates new_allanswers which has the updated information

scale = 1;   % this is the display gain
display_windows = 1;  % whether to show the sample windows on the screen or not

new_allanswers = allanswers;  % initialize the new all answers to be the old ones
[filens, stimns] =  ind2sub(size(new_allanswers),find(strcmp(new_allanswers,'N')));
nbad = length(filens);
myts = mytimeseries;
for ii = 1:nbad   %1:length(results)   % iterate over the different files
    filen = filens(ii);
    stimn = stimns(ii);
    temp = squeeze(results{filen}(:,stimn,:))';
    temp2 = filtfilt(b,a,temp);  % get rid of the DC shifts
    myts.Data = squeeze(results{filen}(:,stimn,:))';  % get the data for this response
    %         myts.Data = temp2;  % get the data for this response
    myts.Time = (1:size(myts.Data,1))/newopts.samplerate;  % the times
    h = initialize_ts_gui(myts,[]);   % this plots the gui with the controls so you can interact with the response
    mat = [squeeze(intresp(:,filen,stimn))'; squeeze(allntotal(:,filen,stimn))']';  % the summary statistics
    set(h,'Name',['file: ' num2str(filen) ', stim: ' num2str(stimn) ,' time: ' num2str(stimtimes{filen}(stimn))]);  % just puts a title on the figure
    set_scales(scale*ones(1,size(myts.Data,2)));  % set the display gains
    set_labels(cellstr(num2str(mat(:,1))));  % labels are the integrated response
    disp(['file: ' num2str(filen) ', stim: ' num2str(stimn)]) % shows information in the command window
    disp('integrated response      nsamples above 3SD')
    disp(num2str(mat))
    if display_windows
        figure(h)
    ...
    options.Resize='on';
    answer=inputdlg({'Good trial?                                         (Y/N)'},['file: ' num2str(ii) ', stim: ' num2str(jj)],1,{'Y'},options);
    new_allanswers(filen,stimn) = answer;  % let the user judge the trial and store the answer
    close
end

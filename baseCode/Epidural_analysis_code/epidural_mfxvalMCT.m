function [R2, varargout] = epidural_mfxvalMCT(binnedData, options)
...
foldLength = round(duration*binsize/nfold);
...
for i=0:nfold-1
    ...
    testDataEnd = testDataStart + options.foldlength;    % this was options.foldlength
    ...
    if options.EMGcascade
        ...
        model = BuildModelMCT(trainData, options); %make a new model (normal code)
        %cd /Users/josephinewallner/Desktop/LabWork/RatData/DecoderResultSets; %used for decoder decay analysis
        %load('N27_180919_DecoderResults.mat'); %used for decoder decay analysis
        %model = resultsSummary.model(3); %used for decoder decay analysis
        %model = model{1};%used for decoder decay analysis
...
%     R2(i+1,:,1) = CalculateR2(TestSigs(round(fillen/binsize):end,:),PredData.preddatabin)';
 ...
 %Ambika commented the following two lines                   
...
AllPredData.ActualData = binnedData.cursorposbin;

AllPredData.mfxval.R2 = R2;
AllPredData.mfxval.vaf= vaf;
AllPredData.mfxval.vaf_fit= vaf_fit;

AllPredData.mfxval.mse= mse;

AllPredData.mfxval.ave_R2 = mean(R2,1);
AllPredData.mfxval.ave_vaf = mean(vaf,1);
AllPredData.mfxval.ave_mse = mean(mse,1);

assignin('base','model',model); %added 4/6/18


% varargout{1} = AllPredData;
% varargout{2} = nfold;
varargout = {vaf, mse, AllPredData, binnedData, vaf_fit, foldLength, nfold};
...
DecoderOptions.foldlength = 72.95;          % Duration of folds (seconds) - must be multiple of 0.05
...
for fileNum=1:length(filenames)
    ...
    kinematicData = [];
    kinematicDataTemp = struct2cell(combinedKinChunk);
    for j = 1:length(kinematicDataTemp)
        ...
        [PredSignal(4*(fileNum-1)+1),ActualData(4*(fileNum-1)+1)] = DecoderWrapperJ(timeframe,combinedSpike,kinematicData,DecoderOptions,labels); %Kinematics from Spikes
        ...
        [PredSignal(4*(fileNum-1)+2),ActualData(4*(fileNum-1)+2)] = DecoderWrapperJ(timeframe,combinedField,kinematicData,DecoderOptions,labels);
        ...
        [PredSignal(4*(fileNum-1)+3),ActualData(4*(fileNum-1)+3)] = DecoderWrapperJ(timeframe,combinedSpike,combinedEMG,DecoderOptions,labels);
        ...
        [PredSignal(4*(fileNum-1)+4),ActualData(4*(fileNum-1)+4)] = DecoderWrapperJ(timeframe,combinedField,combinedEMG,DecoderOptions,labels);

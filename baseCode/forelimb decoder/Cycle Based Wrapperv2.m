...
useind = find((KINtimes<=374.95) & (KINtimes > 0));  % limit the span of data that willbe used
...
% temp = kinematicData.refKinMatrix(useind,19) - kinematicData.refKinMatrix(useind,4);  % this is in case the kinematics aren't already zeroed to the shoulder/hip
...
%N8 - for variable motion analysis
temp = kinematicData.refKinMatrix(useind,19) - kinematicData.refKinMatrix(useind,4);  % this is in case the kinematics aren't already zeroed to the shoulder/hip
...
ind = find(APtimes > usetimes(1) & APtimes < usetimes(end));

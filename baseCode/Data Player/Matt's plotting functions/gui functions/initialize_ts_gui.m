function figurehandle = initialize_ts_gui(sts,varargin)
%  function intiialize_ts_gui(sts,videodata)
...
if nargin > 1  % they sent in a videoData object
    videoData = varargin{1};
end

if nargin > 2 % they sent in an axes
    handles.axishandle = varargin{2};
    figurehandle = get(handles.axishandle,'Parent');
...
handles.maxrangetoshow = 1000000;   % this is the limit on the range of sample numbers to try to show at any given time - saves on the calls

try  % ugly ugly hack 
    totnsamp = size(handles.dat,1);
catch
    totnsamp = size(handles.dat.Data,1);
end
...
if isfield(sts,'nsamp1') && ~isempty(sts.nsamp1)
    nsamp1 = sts.nsamp1;
    nsamp2 = sts.nsamp2;
end
...
handles.threshold = zeros(1,nchan);
handles.real_threshold = zeros(1,nchan);
...
handles.menu_functions = {};
handles.running = 0;
handles.all_ided_times = cell(nchan+1,1);
[handles.all_ided_times{:}] = deal([]);  % initialize everything to a single line
handles.current_trace = nchan;

if ~isempty(videoData)  % there actually is some videoData to display
    handles.dat = Setup_Pulse_Info(handles.dat);  % find the pulses and get started with that
...
for ii = 1:nchan
    ...
    mx(ii) = nanmax(temp - nanmean(temp));
    gain(ii) = 1/(2*mx(ii));
...
handles = add_control(handles,'set_threshold','mouse');
handles = add_control(handles,'analyze_spike_events','menu');
handles = add_control(handles,'playbuttons');
handles = add_control(handles,'idtimes','mouse');
handles = add_control(handles,'zoomx','mouse');
% handles = add_control(handles,'comments','mouse');
handles = add_control(handles,'notes','menu');
handles.notes = {'','',''};
handles = add_control(handles,'save_info','menu');
handles = add_control(handles,'filter_display','menu');
% handles = add_control(handles,'mouse_toggle','pushbutton');
handles = add_control(handles,'mouse_toggle','menu');
handles = add_control(handles,'average_cycles','menu');
handles = add_control(handles,'find_onsets','menu');

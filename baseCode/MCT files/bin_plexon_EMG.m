function [emg_data] = bin_plexon_EMG(emg_data, params)
...
defaultLP = 20;        %Low pass filter
defaultNormalize = 0;  %Normalize EMG
defaultbins = 0;  % no bins defined
...
for E=1:numEMGs
    % Filter EMG data
    tempEMG = double(emg_data.data(:,E));  
    %figure; plot(tempEMG)           
...
%             emgdatabin(:,E) = resample(tempEMG, 1/binsize, emgsamplerate);
    emgdatabin(:,E) = interp1(emg_data.timeframe, tempEMG, timeframe,'linear','extrap');
end
...
emg_data.binned.data = emgdatabin;
emg_data.binned.timeframe = timeframe;
